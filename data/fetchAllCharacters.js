...
async function getAllMarvelCharacters() {
  ...
  // Prima chiamata per ottenere il numero totale di personaggi
  const initialResponse = await fetch(`${baseUrl}?limit=1&${authParams}`);
  const initialData = await initialResponse.json();
  const totalCharacters = initialData.data.total;

  const limit = 100; // Numero di personaggi per ogni chiamata
  let offset = 0;
  const characterData = [];

  while (offset < totalCharacters) {
    const response = await fetch(`${baseUrl}?limit=${limit}&offset=${offset}&${authParams}`);
  ...
  fs.writeFileSync('marvel_characters_with_images.json', JSON.stringify(characterData, null, 2));

  console.log(`File 'marvel_characters_with_images.json' creato con successo. Totale personaggi: ${characterData.length}`);
}

// Esempio di utilizzo
getAllMarvelCharacters().catch(error => console.error(error));
